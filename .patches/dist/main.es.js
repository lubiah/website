...
function assertPart(part, name) {
	if (part.indexOf(path__default.sep) !== -1) {
		throw new Error("`" + name + "` cannot be a path: did not expect `" + path__default.sep + "`");
	}
}
...
function decodeEntity(characters) {
	return own$c.call(characterEntities, characters) ? characterEntities[characters] : false;
}
...
function parseEntities(value, options) {
	...
	for (key in defaults$3) {
		option = options[key];
		settings[key] = option === null || option === undefined ? defaults$3[key] : option;
	}
...
function visitParents(tree, test, visitor, reverse) {
	...
	function factory(node, index, parents) {
		...
		if (typeof value.type === "string") {
			...
			visit.displayName = "node (" + color_1(value.type + (name ? "<" + name + ">" : "")) + ")";
		}
...
function fencedCode(eat, value, silent) {
	...
	while (index < length) {
		...
		if (character === lineFeed$g || (marker === graveAccent$2 && character === marker)) {
			break;
		}
...
function list$1(eat, value, silent) {
	...
	if (character === asterisk$3 || character === plusSign$2 || character === dash$6) {
		...
		if (!queue || !(character === dot$3 || (commonmark && character === rightParenthesis$3))) {
			return;
		}
...
function url(eat, value, silent) {
	...
	while (index < length) {
		...
		if (isDecimal(code) || isAlphabetical(code) || code === dash$3 || code === underscore$4) {
			index++;
			continue;
		}
...
function email(eat, value, silent) {
	...
	if (firstDot === -1 || firstDot === index || code === dash$1 || code === underscore$2) {
		return;
	}
...
function generateFootnotes(h) {
	...
	return h(
		...
		{ className: ["footnotes"] },
		wrap_1(
			[thematicBreak_1(h), list_1(h, { type: "list", ordered: true, children: listItems })],
			true
		)
	);
}
...
function body(node) {
	...
	return (
		!head ||
		(!unistUtilIs("comment", head) && !whiteSpaceLeft_1(head) && !hastUtilIsElement(head, meta))
	);
}
...
var index$1 = ["script", "style", "pre", "textarea"];
...
var dangerous$1 = ["cent", "copy", "divide", "gt", "lt", "not", "para", "times"];
...
var constants = {
	name: [
		[name, name.concat(DQ$1, SQ$1, GR)],
		[name.concat(NULL, DQ$1, SQ$1, LT), name.concat(NULL, DQ$1, SQ$1, LT, GR)]
	],
	unquoted: [
		[unquoted$1, unquotedSafe],
...
const attributeValue =
	"(?:" + unquoted + "|" + singleQuoted + "|" + doubleQuoted + "|" + jsProps + ")";
...
function tokenizerFactory(childType, expression) {
	...
	function tokenizer(node) {
		...
		while (++index < length) {
			if (
				index === lastIndex ||
				(tokens[index].type === childType && expression.test(nlcstToString_1(tokens[index])))
			) {
				first = tokens[start];
				last = tokens[index];
		...
		if (prev && prev.type === "WordNode") {
			...
			while (siblings[++position]) {
				...
				if (sibling.type === "WordNode") {
					...
					(sibling.type === "SymbolNode" || sibling.type === "PunctuationNode") &&
...
var mergeInitialLowerCaseLetterSentences_1 = unistUtilModifyChildren(
...
function quotes(node, index, parent) {
	...
	if (
		...
		prev &&
		next &&
		(prev.type === whiteSpace || prev.type === punctuation || prev.type === symbol) &&
		next.type === word
	) {
...
function transform_hast({ layout, layout_mode }) {
...
const code_highlight = (code, lang) => {
	...
	if (!process.browser) {
		...
		const highlighted = escape_svelty(
			_lang ? Prism.highlight(code, Prism.languages[_lang.name], _lang.name) : escapeHtml_1(code)
		);
...
function transform(
	...
	if (smartypants) {
		toMDAST.use(smartypants_transformer, typeof smartypants === "boolean" ? {} : smartypants);
	}
...
export {
	code_highlight as code_highlighter,
	_compile as compile,
